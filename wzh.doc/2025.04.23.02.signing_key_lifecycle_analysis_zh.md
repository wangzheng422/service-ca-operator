...
*   **生命周期管理:** `signing-key` Secret（包含 service-ca 的主 CA 证书和私钥）的**轮换**是由 `service-ca-operator` 自身管理的，具体逻辑在 `pkg/operator/rotate.go` 中实现。
    *   **轮换触发条件:**
        *   **时间驱动:** 当现有 CA 证书的剩余有效期低于其总有效期的一半时（即 `NotAfter` 时间早于当前时间加上 `minimumTrustDuration`，默认为 13 个月），会自动触发轮换。
        *   **强制轮换:** 可以通过设置 `UnsupportedServiceCAConfig` 中的 `ForceRotation.Reason` 来强制触发轮换。
    *   **初始创建:** 虽然轮换由 Operator 管理，但 `signing-key` Secret 的**初始创建**可能仍然由集群的证书管理基础设施（如 cert-manager 或 OpenShift 安装过程）或其他外部机制负责。Operator 在启动时会检查并确保证书存在，如果不存在会尝试创建（如 `pkg/operator/starter.go` 中的 `ensureSigningCertKeyPair`）。
*   **加载与消费:**
